...
impl Operation {
    fn doit(&self, item: i32) -> i32 {
        let roper = match self.operand {
            Operand::Old => item,
            Operand::Number(n) => n
        };
        match self.operator {
            Operator::Add => (item + roper) / 3,
            Operator::Mult => (item * roper) / 3
        }
    }
}
...
impl Monkey {
    pub fn turn(&mut self) -> Vec<(i32, i32)>{
        let mut item_to_next: Vec<(i32, i32)> = Vec::new();
        self.num_inspects += self.items.len().clone() as i32;
        while self.items.len() > 0 {
            let mut item = self.items.remove(0);
            item = self.operation.doit(item);
...
fn main() {
    ...
    for _ in 0..20 {
        ...
        for i in 0..num_monkeys {
            ...
            let items_to_next = the_monkey.turn();
    ...
    let answer = monkey_vec.iter().map(|m| m.num_inspects).rev().take(2).collect::<Vec<i32>>();
    ...
    for line in instruction.lines().map(|l| l.trim()).collect::<Vec<&str>>() {
        match line.trim() {
            ...
            _ if line.starts_with("Starting items") => {
                ...
                monkey.items = split[1].split(", ").map(|i| i.parse().unwrap()).collect::<Vec<i32>>();
            },
